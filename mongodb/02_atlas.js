exports = async function(orderId, statusId) {
    ...
    try {
      ...
      const collection = context.services.get("mongodb-atlas").db("bookstore").collection("orders");
      
      // Find the order document
      const order = await collection.findOne({ "_id": BSON.ObjectId(orderId) });
      ...
      if (currentStatus !== statusId && statusId >= 1 && statusId <= 6) {
        ...
        const message = currentStatus === statusId 
